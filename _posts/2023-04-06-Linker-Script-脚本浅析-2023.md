...
```TXT
11. ASSERT(EXP, MESSAGE)：如果 EXP 不为真，终止连接过程
```

```TXT
12. EXTERN(SYMBOL SYMBOL ...)：在输出文件中增加未定义的符号，如同连接器选项 -u
```

```TXT
13. FORCE_COMMON_ALLOCATION：为 common symbol(通用符号) 分配空间，即使用了-r 连接选项也为其分配
```
